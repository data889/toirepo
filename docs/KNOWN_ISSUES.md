...
## M12 env trap (2026-04-22)

### `tsx --env-file=.env.local` 覆盖外部 `export DATABASE_URL` → 所有 prod 意图落入本地 docker

**症状**（Ming 发现，最重要的一条 debt）：从 M8 DeepL 真执行到 M12 全球 OSM
import，Ming 每次都 `export DATABASE_URL='<prod Supabase URI>'` 再跑 pnpm
脚本，但 **prod 从 M10 P1 起一直 10,107 行未动**，本地 docker 却涨到
365,759 行。M12 listByBbox 在 Tokyo 有数据（M11 导入那时走的路径没受此
trap 影响）、海外 bbox 全空 —— 不是 location NULL，是**这些行从未到 prod**。

**根因**：`package.json` 中
`tsx --env-file=.env.local scripts/osm-import-global.ts …`
的 Node/tsx `--env-file` 语义在本机版本组合下 **会覆盖外部已 export 的
`DATABASE_URL`**（官方 Node 22.7+ 文档说不覆盖，但实测覆盖 —— tsx 可能
自己实现了 dotenv 加载）。因此：
- `export DATABASE_URL='<prod>' && pnpm osm:import-global` 实际连本地 docker
- `DATABASE_URL='<prod>' pnpm deepl:translate --execute` 同上
- 所有 prod 意图脚本（osm:import, osm:import-global, deepl:translate,
  osm:dryrun）都落入本地

**影响范围（已造成的真实偏差）**：
- 本地 docker: 365,759 行（10k M11 Tokyo + 355k M12 全球 OSM + DeepL
  三语翻译全齐）
- 生产 Supabase: 10,107 行（仅 M11 Tokyo + M10 P1 时的 seed / user rows）
- 海外城市在 prod 上零 markers

**修复**（commit: `fix(tooling): decouple prod scripts from .env.local`）：

1. **`scripts/lib/env-boot.ts`** —— 共享 shim，仅在 `process.env.DATABASE_URL`
   未设时才 dotenv-load `.env.local`。外部 export 永远赢。
2. **package.json 改 6 个 pnpm 脚本**：`osm:dryrun` / `osm:import` /
   `osm:import:dryrun` / `osm:import-global` / `osm:import-global:dryrun` /
   `deepl:translate` —— 去掉 `--env-file=.env.local`。
3. **每个 prod-capable 脚本开头**调 `announceTarget({ graceSeconds: 5 })`
   打印 `🎯 Target: PROD|LOCAL — <host>` + prod 给 5s Ctrl+C 取消窗口。
4. **`scripts/sync-local-to-prod.ts`** + `pnpm sync:local-to-prod` —— 一把
   把本地 365k 迁到 prod。双 Prisma client (LOCAL_DATABASE_URL + DATABASE_URL)，
   osmId upsert，slug 碰撞预检，update path 只动 OSM 字段不碰 prod 现有
   admin state（status / publishedAt / cleanliness / reviewCount 等）。

**保留 `--env-file=.env.local` 的脚本**（dev-only，力求本地化）：
`seed` / `test:moderation` / `sentry:test` / `pmtiles:*` / `auth:check`。
`seed` 尤其危险（不能错入 prod），`--env-file` 此处充当安全封印。

**防回归**：`announceTarget` 的 `🎯 Target` 输出是操作手心肌。每次跑 prod
意图命令都应肉眼确认 `PROD — <supabase-host>`，否则 Ctrl+C。
