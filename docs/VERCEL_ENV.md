...
### 1.5.1 Sentry 错误监控 (M10 P2)

| Variable | 值 |
|---|---|
| `NEXT_PUBLIC_SENTRY_DSN` | sentry.io → Project → Settings → Client Keys (DSN) 复制 |
| `SENTRY_AUTH_TOKEN` | User Settings → Auth Tokens → Create Token（勾 `project:releases` + `project:write`，**不要**给 account-wide 权限）|
| `SENTRY_ORG` | Sentry org slug，本项目 = `jandoo` |
| `SENTRY_PROJECT` | Sentry project slug，本项目 = `javascript-nextjs`（创建时默认名保留）|

Prod + Preview 都配同一套。`SENTRY_AUTH_TOKEN` 仅构建时用（源 map 上传），runtime 不需要；即使如此放 Production Only + Preview 即可，**不要** Development（本地不上传源 map）。
